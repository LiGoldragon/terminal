...
- one checked-in generated daemon module, `src/schema/daemon.rs`, produced
  from `schema/daemon.schema` by `schema-rust`, carrying the async
  multi-listener process shell and its meta-listener tier. The retired stack
  also emitted `signal`, `sema` and `nexus` modules here — a further 5014
  lines whose only reader was a test written to exercise them. The contract
  they anticipated now exists as `signal-terminal` 2.0.1, generated by
  ethos-zero in the contract's own repository, so they were deleted rather
  than regenerated;
...
its typed configuration from one binary rkyv file at startup, refuses inline
text while the argument is read and a non-archive file at decode, binds its
communication and supervision sockets, starts its terminal session actors, and reports readiness only after
...
`terminal` is the ordinary component CLI. It accepts one inline Datom
`signal-terminal::Query`, sends a length-prefixed Signal frame to the
component communication socket, and renders the typed reply as Datom text.

`meta-terminal` is the meta component CLI. It accepts one inline Datom
`meta-signal-terminal::Query`, sends it to the terminal meta socket, and
renders the typed reply as Datom text.
